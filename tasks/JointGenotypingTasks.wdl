...
task CheckSamplesUnique {
  ...
  command {
    set -euo pipefail
    if [[ $(grep 'chr4' ~{sample_name_map} | cut -f 1 | wc -l) -ne $(grep 'chr4' ~{sample_name_map} | cut -f 1 | sort | uniq | wc -l) ]]
    then
      echo "Samples in the sample_name_map are not unique" 1>&2
      exit 1
    elif [[ $(grep 'chr3' ~{sample_name_map} | cut -f 1 | wc -l) -lt ~{sample_num_threshold} ]]
    then
